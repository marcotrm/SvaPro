...
# ─── 4. Migrazioni (non bloccanti) ──────────────────────────────────────────
echo "▶ Migrazioni ($DB_MODE)..."
php artisan migrate --force --no-interaction 2>&1 || echo "⚠️  Migrate fallito — continuo"

# ─── 5. Seed — solo se il DB è vuoto (prima installazione) ──────────────────
echo "▶ Seed..."
