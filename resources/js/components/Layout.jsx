  ...
  return (
    <div className={`app ${isSidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      ...
      <aside className={`sidebar ${isSidebarCollapsed ? 'collapsed' : ''}`}>
        ...
        <nav className="nav">
          {navGroups.map(group => (
            <div key={group.label} className={`nav-group ${expandedNavGroups[group.label] ? 'expanded' : ''}`}>
              <div className="nav-label" onClick={() => toggleNavGroup(group.label)}>
                <span>{trLabel(group.label)}</span>
                {!isSidebarCollapsed && <span className="chevron">▼</span>}
              </div>
                ...
                {group.items.map(item => {
                  ...
                  return (
                    <a
                      ...
                      href={item.href}
                      title={isSidebarCollapsed ? trLabel(item.label) : ''}
                      onMouseEnter={() => prefetchRoute(item.href)}
                      onClick={e => { e.preventDefault(); navigate(item.href); }}
                    >
                      {item.icon}
                      <span className="menu-label">{trLabel(item.label)}</span>
          ...
          {isSuperAdmin && (
            <div className={`nav-group ${expandedNavGroups['Superadmin'] ? 'expanded' : ''}`}>
              <div className="nav-label" onClick={() => toggleNavGroup('Superadmin')}>
              ...
              <div className="nav-group-items">
                ...
                >
                  ...
                  <span className="menu-label">Control Tower</span>
                </a>
                <a
                  className={`nav-item${location.pathname === '/roles-permissions' ? ' active' : ''}`}
                  href="/roles-permissions"
                  title={isSidebarCollapsed ? 'Ruoli & Permessi' : ''}
                ...
                >
                  ...
                  <span className="menu-label">Ruoli & Permessi</span>
                </a>
                <a
                  className={`nav-item${location.pathname === '/settings' ? ' active' : ''}`}
                  href="/settings"
                  title={isSidebarCollapsed ? 'Impostazioni' : ''}
                ...
                >
                  ...
                  <span className="menu-label">Impostazioni</span>
                </a>
              </div>
            </div>
          )}
        ...
        <div className="sidebar-footer">
          {userPanelOpen && !isSidebarCollapsed && (
          ...
          <button className="user-card" onClick={() => setUserPanelOpen((prev) => !prev)} style={{width: '100%', border: 'none', background: 'transparent', padding: 0}}>
            ...
            {!isSidebarCollapsed && (
              <>
                ...
                <svg style={{ marginLeft: 'auto', opacity: .55, transform: userPanelOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} width="14" height="14" viewBox="0 0 20 20" fill="currentColor">
                  <path d="M6 8l4 4 4-4"/>
                </svg>
              </>
            )}
